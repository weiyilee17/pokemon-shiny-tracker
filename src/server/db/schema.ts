...
export const pokemonCaptured = mysqlTable(
  "pokemon_captured",
  {
    ...
    pokemonId: smallint("pokemonId").notNull(),
  },
  (pc) => ({
    compoundKey: primaryKey({ columns: [pc.userId, pc.pokemonId] }),
  }),
);

export const pokemonCapturedRelations = relations(
  pokemonCaptured,
  ({ one }) => ({
    user: one(users, {
      fields: [pokemonCaptured.userId],
      references: [users.id],
    }),
    pokemon: one(pokemons, {
      fields: [pokemonCaptured.pokemonId],
